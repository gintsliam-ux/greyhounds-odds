...
export default function RaceDetail() {
  const { raceId, sport: sportParam } = useParams<{ raceId: string; sport: string }>()
  ...
  const load = useCallback(
    async (silent: boolean) => {
      ...
      try {
        const r = await fetchRace(sport, raceId)
        ...
        if (m) {
          fetchMeetings(sport, m.date)
            .then((ms) => {
              if (!cancelledRef.current) setSameDayMeetings(ms)
            })
            .catch(() => {})
        }
        const o = await fetchOddsForRunners(sport, rs.map((x) => x.id))
        if (cancelledRef.current) return
        setOdds(o)
      } catch (e) {
        if (!cancelledRef.current && !silent) setError((e as Error).message)
      } finally {
  ...
  useEffect(() => {
    if (!raceId) return
    const racesTable = SPORT_TABLES[sport].races
    const channel = supabase
      .channel(`race-${sport}-${raceId}`)
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: racesTable, filter: `id=eq.${raceId}` },
        (payload) => {
  ...
  const drift = (o: Odds | undefined, bookmaker?: string): number | null => {
    if (!o) return null
    const baselineCp: OddsCheckpoint = bookmaker?.toLowerCase() === 'betfair' ? '30m' : 'open'
    const baseline = getOdd(o, baselineCp)
  ...
  return (
    <div className="space-y-4">
      <div>
        <Link
          to={meeting ? `/${sport}/meetings/${meeting.id}` : `/${sport}/today`}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            onClick={() => prevRace && navigate(`/${sport}/races/${prevRace.id}`)}
            disabled={!prevRace}
          ...
          <button
            onClick={() => nextRace && navigate(`/${sport}/races/${nextRace.id}`)}
            disabled={!nextRace}
            className="p-1.5 rounded border border-gray-800 text-gray-400 hover:bg-gray-800 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
          ...
          <button
            onClick={() => load(true)}
            disabled={refreshing}
            className="p-1.5 rounded border border-gray-800 text-gray-400 hover:bg-gray-800 hover:text-white disabled:opacity-50 transition-colors ml-1"
            aria-label="Refresh"
            title="Refresh"
          >
        ...
        <div className="flex gap-1 overflow-x-auto scrollbar-none -mx-1 px-1">
          {meetingRaces.map((r) => {
            ...
            return (
              <button
                key={r.id}
                onClick={() => navigate(`/${sport}/races/${r.id}`)}
      ...
      <div className="space-y-5">
        {activeRunners.length === 0 && scratchedRunners.length === 0 && (
          <p className="text-sm text-gray-400">No runners.</p>
        )}
        {[...activeRunners, ...scratchedRunners].map((runner) => {
          const scratched = !!runner.scratched_at
          const books = oddsByRunnerBook[runner.id] || {}
          const num = runnerNumber(runner)
          const barrier = sport === 'greyhounds' ? null : runnerBarrier(runner)
          return (
            <div
              key={runner.id}
              className={`bg-gray-900 border border-gray-800 rounded-xl overflow-hidden ${scratched ? 'opacity-60' : ''}`}
            >
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800">
                <div className="flex items-center gap-3">
                  <span className={`w-7 h-7 rounded-full bg-gray-800 flex items-center justify-center text-xs font-semibold ${scratched ? 'text-gray-500' : 'text-gray-200'}`}>
                    {num ?? '—'}
                  </span>
                  <div className="flex flex-col">
                    <span className={`font-medium ${scratched ? 'text-gray-500 line-through' : 'text-white'}`}>
                      {runner.name}
                ...
                {scratched && (
                  <span className="text-[10px] uppercase tracking-wider text-red-400 font-semibold shrink-0">
                    Scratched
                  </span>
                )}
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-[10px] uppercase tracking-wider text-gray-500">
                      <th className="text-left font-medium px-3 sm:px-4 py-2 sticky left-0 bg-gray-900 z-10">Bookmaker</th>
                      {CHECKPOINTS.map((cp) => (
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {bookmakers.map((bk) => {
                      const o = books[bk]
                      const d = drift(o, bk)
                      const isBetfair = bk.toLowerCase() === 'betfair'
                      const bfField: 'blend_fair' | 'blend_fair_place' = market === 'place' ? 'blend_fair_place' : 'blend_fair'
                      const bf = isBetfair
                        ? (Object.values(books).find((x) => x?.[bfField] != null)?.[bfField] ?? null)
                        : null
                      return (
                        <Fragment key={bk}>
                          <tr>
                            ...
                            <td
                              ...
                              title={isBetfair ? '% change from 30m' : '% change from open'}
                            >
                          ...
                          {bf != null && (
                            <tr className="bg-gray-900/50">
                              <td className="px-3 sm:px-4 py-2 text-[10px] uppercase tracking-wider text-emerald-400 font-semibold sticky left-0 bg-gray-900/95 z-10">
                                Blend Fair
                              </td>
                              {CHECKPOINTS.map((cp) => (
                                <td key={cp} className="px-2 sm:px-3 py-2 text-right font-mono tabular-nums text-emerald-300">
