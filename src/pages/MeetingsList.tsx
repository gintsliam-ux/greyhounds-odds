...
import { SPORT_LABELS, sportFromPath } from '../lib/sport'
...
export default function MeetingsList({ date }: { date: string }) {
  const params = useParams<{ date?: string; sport?: string }>()
  const sport = sportFromPath(params.sport)
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-white mb-1">{SPORT_LABELS[sport].singular} Meetings</h1>
      ...
      {loading ? (
        <div className="space-y-6">
          {[0, 1].map((k) => (
            <div key={k} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
              ...
              {Array.from({ length: 5 }).map((_, i) => (
                ...
                >
                  <div className="h-4 w-28 bg-gray-800 rounded animate-pulse shrink-0" />
                  <div className="flex-1 flex items-center justify-end gap-2 overflow-hidden">
                    {Array.from({ length: 8 }).map((_, j) => (
                      <div key={j} className="h-5 w-10 sm:w-14 bg-gray-800 rounded animate-pulse shrink-0" />
                    ))}
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-6">
          {([
            ...
            return (
              <div key={g.key} className="bg-gray-900 border border-gray-800 rounded-xl overflow-x-auto scrollbar-none">
                <table className="w-full min-w-[720px] text-sm table-fixed">
                  <colgroup>
                    <col className="w-40 sm:w-60" />
                  ...
                  <thead>
                    <tr className="border-b border-gray-800">
                      <th className="text-left text-[11px] font-semibold text-gray-300 uppercase tracking-wider px-4 py-3 sticky left-0 bg-gray-900 z-10">
                        {g.label}
                      </th>
                      {Array.from({ length: max }, (_, i) => i + 1).map((n) => (
                        <th key={n} className="text-center text-[11px] font-medium text-gray-400 uppercase tracking-wider px-2 py-3">
                          R{n}
                        </th>
                      ))}
                    </tr>
                    ...
                    {g.list.map((m) => {
                      ...
                      return (
                        <tr key={m.id} className="hover:bg-gray-800/50 transition-colors">
                          <td className="px-4 py-3 sticky left-0 bg-gray-900 z-10 whitespace-nowrap overflow-hidden text-ellipsis">
                            <Link to={`/${sport}/meetings/${m.id}`} className="font-medium text-white hover:text-emerald-300 transition-colors">
                              {m.track}
                            </Link>
                          ...
                          {Array.from({ length: max }, (_, i) => i + 1).map((n) => {
                            ...
                            return (
                              <td key={n} className="px-2 py-3 text-center">
                                {r ? <RaceBadge race={r} sport={sport} /> : <span className="text-gray-700">—</span>}
                              </td>
                            )
                          })}
                        </tr>
